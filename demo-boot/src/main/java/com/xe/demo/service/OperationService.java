...
import com.xe.demo.mapper.AuthOperationMapper;
...
public class OperationService extends AbstratService<AuthOperation> {
	...
	@Autowired
	private AuthOperationMapper operationMapper;
...
//		}
	...
	@ServiceLog("新增操作")
	public AjaxResult saveOp(AuthOperation op) {
		String result = null;
		AuthOperation tempOPeration = operationMapper.queryByOphref(op.getOphref());
		if (null == tempOPeration) {
			save(op);
		} else {
			result = "操作已存在";
		}
